...
export class SalesReturnService {
  ...
  async context(invoiceId: bigint) {
    ...
    if (invoice.status !== 'COMPLETED')
      throw new HttpException(
        'Hanya Sales Invoice COMPLETED yang dapat diretur.',
        HttpStatus.CONFLICT,
      );
    if (invoice.documentType === 'OPENING_BALANCE')
      throw new HttpException(
        'Saldo awal piutang tidak memiliki barang yang dapat diretur.',
    ...
    return {
      ...
      details: invoice.details.map((detail) => {
        const returned = detail.salesReturnDetails.reduce(
          (sum, row) => sum.add(row.quantity),
          ZERO,
        );
        const returnedBonus = detail.salesReturnDetails.reduce(
        ...
        return {
          ...
          returnableBonusQuantity: Number(
            detail.bonusQuantity.sub(returnedBonus),
          ),
  ...
  async list(page = 1, limit = 20) {
    ...
    return {
      data: rows.map((row) => ({
        salesReturnId: row.salesReturnId.toString(),
        salesReturnNumber: row.salesReturnNumber,
        salesInvoiceId: row.salesInvoiceId.toString(),
  ...
  async find(id: bigint) {
    ...
    return {
      ...
      replacementSalesInvoiceId:
        row.replacementSalesInvoiceId?.toString() ?? null,
      replacementSalesInvoiceNumber:
        row.replacementSalesInvoice?.salesInvoiceNumber ?? null,
      customerName:
        row.customer?.customerName ?? row.salesInvoice.customerName ?? 'Guest',
      returnDate: row.returnDate,
      returnTotal: Number(row.returnTotal),
      status: row.status,
      resolutionType: row.resolutionType,
      receivableOffsetAmount: Number(row.receivableOffsetAmount),
      replacementCreditAmount: Number(row.replacementCreditAmount),
      refundAmount: Number(row.refundAmount),
      refundAccountName: row.financialAccount?.accountName ?? null,
      ...
      details: row.details.map((detail) => ({
        salesReturnDetailId: detail.salesReturnDetailId.toString(),
        productName: detail.productUnit.product.productName,
        unitName: detail.productUnit.unit.unitName,
        ...
        unitPrice: Number(detail.unitPrice),
        subtotal: Number(detail.subtotal),
        returnCostTotal: Number(detail.returnCostTotal),
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const requested = new Map(
          dto.items.map((item) => [item.salesInvoiceDetailId, item]),
        );
        const detailIds = invoice.details.map(
          (item) => item.salesInvoiceDetailId,
        );
        const previous = await tx.salesReturnDetail.groupBy({
          by: ['salesInvoiceDetailId'],
          where: {
            salesInvoiceDetailId: { in: detailIds },
        ...
        const previousMap = new Map(
          previous.map((row) => [
            row.salesInvoiceDetailId.toString(),
            row._sum,
          ]),
        );
        const items = invoice.details.flatMap((detail) => {
          ...
          if (
            quantity.lessThan(0) ||
            bonusQuantity.lessThan(0) ||
            quantity.add(bonusQuantity).equals(0)
          )
            return [];
          const old = previousMap.get(detail.salesInvoiceDetailId.toString());
          assertReturnQuantity({
            sold: detail.quantity,
            soldBonus: detail.bonusQuantity,
            previouslyReturned: old?.quantity ?? ZERO,
            previouslyReturnedBonus: old?.bonusQuantity ?? ZERO,
            requested: quantity,
            requestedBonus: bonusQuantity,
          });
        ...
        if (!items.length)
          throw new HttpException(
            'Minimal satu barang harus diretur.',
            HttpStatus.UNPROCESSABLE_ENTITY,
          );
        ...
        const header = await tx.salesReturn.create({
          data: {
            ...
            details: {
              create: items.map(
                ({ detail, item, quantity, bonusQuantity, subtotal }) => ({
        ...
        await writeAuditLog(tx, {
          ...
          entityNumber: number,
          source: 'API',
          changedFields: changedFields(null, header),
        });
        if (dto.status === 'COMPLETED')
          await this.completeTx(tx, actorId, header.salesReturnId, dto);
        return {
          salesReturnId: header.salesReturnId.toString(),
          salesReturnNumber: number,
        };
      },
      { timeout: 30_000 },
  ...
  private async completeTx(
    ...
    if (row.status !== 'DRAFT')
      throw new HttpException(
        'Hanya Sales Return DRAFT yang dapat diselesaikan.',
        HttpStatus.CONFLICT,
      );
    let totalCost = ZERO;
    for (const detail of row.details) {
      const sourceMovement = await tx.inventoryMovement.findUnique({
        where: { salesInvoiceDetailId: detail.salesInvoiceDetailId },
      });
      if (!sourceMovement)
        throw new HttpException(
          'Jejak FIFO Sales Invoice tidak ditemukan.',
          HttpStatus.CONFLICT,
        );
      ...
      if (
        !parentUnit ||
        parentUnit.productUnitId !== sourceMovement.productUnitId
      )
        throw new HttpException(
      ...
      const movement = await tx.inventoryMovement.create({
        data: {
          ...
          productUnitId: sourceMovement.productUnitId,
          direction: 'IN',
          quantity: restoreQty,
          movementType: INVENTORY_MOVEMENT_TYPES.SALES_RETURN_IN,
          originType: INVENTORY_ORIGIN_TYPES.SALES_RETURN,
          originId: id,
          originNumber: row.salesReturnNumber,
          salesReturnDetailId: detail.salesReturnDetailId,
      ...
      const allocations = await tx.fifoLayerTransaction.findMany({
        where: {
          inventoryMovementId: sourceMovement.inventoryMovementId,
          direction: 'OUT',
        },
        orderBy: { fifoLayerTransactionId: 'desc' },
      ...
      for (const allocation of allocations) {
        ...
        const capacity = Prisma.Decimal.max(
          ZERO,
          allocation.quantity.sub(alreadyRestored._sum.quantity ?? ZERO),
        );
        ...
        const layer = await tx.fifoLayer.findUniqueOrThrow({
        ...
        await tx.fifoLayerTransaction.create({
          data: {
            ...
            quantityAfter: layer.remainingQty.add(take),
            createdBy: actorId,
          },
        });
        detailCost = detailCost.add(cost);
        remaining = remaining.sub(take);
      }
      if (remaining.greaterThan(0))
        throw new HttpException(
          'Jumlah retur melebihi alokasi FIFO asli.',
          HttpStatus.CONFLICT,
        );
      await tx.salesReturnDetail.update({
        where: { salesReturnDetailId: detail.salesReturnDetailId },
        data: { returnCostTotal: detailCost },
      });
      totalCost = totalCost.add(detailCost);
      await tx.inventoryStock.update({
        where: { productUnitId: sourceMovement.productUnitId },
        data: {
          actualQty: { increment: restoreQty },
          availableQty: { increment: restoreQty },
        },
      });
    }
    const { receivableOffset: offset, residualCredit: residual } =
      resolveReturnSettlement(
        row.returnTotal,
        row.salesInvoice.outstandingAmount,
      );
    ...
    if (row.resolutionType === 'REPLACEMENT') {
      if (!dto.replacementInvoice)
        throw new HttpException(
          'Data Sales Invoice barang pengganti wajib diisi.',
          HttpStatus.UNPROCESSABLE_ENTITY,
        );
      const replacementDto = {
        ...dto.replacementInvoice,
        customerId: row.salesInvoice.customerId?.toString(),
        partyType: row.salesInvoice.partyType,
        customerName: row.salesInvoice.customerName ?? undefined,
        status: 'COMPLETED' as const,
        salesOrderId: undefined,
      };
      const replacementTotal = await this.sales.quoteInvoiceTx(
        tx,
        replacementDto,
      );
      replacementCredit = Prisma.Decimal.min(residual, replacementTotal);
      cashRefund = residual.sub(replacementCredit);
      const replacement = await this.sales.createInvoiceTx(
        tx,
        actorId,
        replacementDto,
        replacementCredit,
    ...
    if (row.customerId && offset.greaterThan(0)) {
      await tx.customerFinancialSummary.update({
        where: { customerId: row.customerId },
        data: {
          outstandingAmount: { decrement: offset },
          currentAmount: { decrement: offset },
        },
      });
    ...
    if (cashRefund.greaterThan(0)) {
      if (!dto.financialAccountId)
        throw new HttpException(
          'Akun Kas/Bank wajib dipilih untuk pengembalian uang.',
          HttpStatus.UNPROCESSABLE_ENTITY,
        );
      ...
      if (!account?.isActive || account.currentBalance.lessThan(cashRefund))
        throw new HttpException(
          'Saldo akun pengembalian tidak mencukupi atau akun tidak aktif.',
          HttpStatus.CONFLICT,
        );
      const method =
        dto.refundPaymentMethod === 'LAINNYA'
          ? dto.otherRefundPaymentMethod?.trim()
          : dto.refundPaymentMethod;
      if (!method)
        throw new HttpException(
          'Metode pengembalian wajib dipilih.',
          HttpStatus.UNPROCESSABLE_ENTITY,
        );
      const ft = (
        await postFinancialMovement(tx, {
          ...
          referenceType: 'SALES_RETURN',
          referenceId: id,
          referenceNumber: row.salesReturnNumber,
          transactionDate: row.returnDate,
          description: `Refund ${row.salesReturnNumber}`,
          note: row.note ?? undefined,
          createdBy: actorId,
          counterChartAccountCode: '4191',
        })
    ...
    if (totalCost.greaterThan(ZERO)) {
      await postJournalEntry(tx, {
        postingKey: `INVENTORY:SALES_RETURN:${id}`,
        transactionDate: row.returnDate,
        description: `Pemulihan persediaan ${row.salesReturnNumber}`,
        sourceType: 'SALES_RETURN',
        sourceId: id,
        sourceNumber: row.salesReturnNumber,
        createdBy: actorId,
        lines: [
          { chartAccountCode: '1301', debitAmount: totalCost },
          { chartAccountCode: '5101', creditAmount: totalCost },
        ],
      });
    }
    const nonCashSettlement = offset.add(replacementCredit);
    if (nonCashSettlement.greaterThan(ZERO)) {
      await postJournalEntry(tx, {
        postingKey: `NON_CASH:SALES_RETURN:${id}`,
        transactionDate: row.returnDate,
        description: `Pengurang penjualan ${row.salesReturnNumber}`,
        sourceType: 'SALES_RETURN',
        sourceId: id,
        sourceNumber: row.salesReturnNumber,
        createdBy: actorId,
        lines: [
          { chartAccountCode: '4191', debitAmount: nonCashSettlement },
          { chartAccountCode: '1201', creditAmount: nonCashSettlement },
        ],
      });
    }
    await tx.salesInvoice.update({
      where: { salesInvoiceId: row.salesInvoiceId },
      data: {
        outstandingAmount: row.salesInvoice.outstandingAmount.sub(offset),
        statusPayment: row.salesInvoice.outstandingAmount.sub(offset).equals(0)
          ? 'PAID'
          : row.salesInvoice.statusPayment,
      },
    });
    const completed = await tx.salesReturn.update({
      ...
      data: {
        ...
        approvedAt: new Date(),
        approvedBy: actorId,
        receivableOffsetAmount: offset,
        replacementSalesInvoiceId,
        replacementCreditAmount: replacementCredit,
        refundAmount: cashRefund,
        financialAccountId: cashRefund.greaterThan(0)
          ? BigInt(dto.financialAccountId!)
          : null,
    ...
    await writeActivityLog(tx, {
      ...
      entityType: 'SALES_RETURN',
      entityId: id,
      entityNumber: row.salesReturnNumber,
    ...
    await writeAuditLog(tx, {
      ...
      entityNumber: row.salesReturnNumber,
      source: 'API',
      changedFields: changedFields(row, completed),
    });
  }
}
