...
export default function FinanceAccountingReports({ mode, dateFrom, dateTo, onDateChange }: { mode: "JOURNAL" | "PROFIT_LOSS"; dateFrom: string; dateTo: string; onDateChange: (dateFrom: string, dateTo: string) => void }) {
  ...
  return <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
    ...
    {mode === "JOURNAL" ? <>
      <div className="grid gap-2 border-b p-3 sm:grid-cols-[minmax(0,1fr)_minmax(220px,320px)_auto]"><div className="relative"><Search className="absolute left-2 top-2.5 h-4 w-4 text-slate-400"/><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Cari jurnal, akun, atau dokumen" className="w-full rounded-lg border py-2 pl-8 pr-2 text-sm"/></div><select value={chartAccountId} onChange={(event) => { setChartAccountId(event.target.value); setPagination((old) => ({ ...old, page: 1 })); }} className="w-full rounded-lg border px-2 py-2 text-sm"><option value="">Semua akun jurnal</option>{chartAccounts.map((account) => <option key={account.chartAccountId} value={account.chartAccountId}>{account.accountCode} · {account.accountName}</option>)}</select><button disabled={!journals.length} onClick={() => void exportJournalPdf(journals, dateFrom, dateTo).catch((reason) => setError(parseApiError(reason)))} className="rounded-lg border border-blue-200 px-3 py-2 text-xs font-bold text-blue-700 disabled:opacity-40"><Download className="mr-1 inline h-4 w-4"/>Export PDF halaman ini</button></div>
