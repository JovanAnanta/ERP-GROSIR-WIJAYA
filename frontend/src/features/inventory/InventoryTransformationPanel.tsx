...
export default function InventoryTransformationPanel({ onNavigate }: { onNavigate: (section: InventorySection) => void }) {
  ...
  return <div className="min-h-full bg-slate-50 p-3 sm:p-6">
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <Button onClick={openCreate} className="bg-[#326dc8] text-white shadow-sm shrink-0"><Plus className="mr-2 h-4 w-4"/>Buat Transformation</Button>
    </div>
    ...
    <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogOverlay className="fixed inset-0 z-50 bg-black/50"/><DialogContent className="z-[60] flex max-h-none max-w-none flex-col overflow-hidden bg-white p-5" style={{ width: '1250px', maxWidth: '96vw', height: '92vh' }}><DialogHeader><DialogTitle>Buat Inventory Transformation</DialogTitle></DialogHeader><div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold">Tanggal<input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="mt-1 h-10 w-full rounded-md border px-3"/></label><label className="text-xs font-bold">Catatan dokumen<input value={note} onChange={(e) => setNote(e.target.value)} className="mt-1 h-10 w-full rounded-md border px-3" placeholder="Opsional"/></label></div>{error && <div className="mt-3 min-h-10 rounded-md border border-rose-200 bg-rose-50 p-2 text-xs font-semibold text-rose-700">{error}</div>}<div className="erp-scroll-table mt-3 min-h-0 flex-1 overflow-auto rounded-xl border"><table className="w-full min-w-[1180px] text-xs"><thead className="sticky top-0 bg-slate-100"><tr><th className="p-3 text-left">Bahan</th><th>Unit</th><th>Qty bahan</th><th></th><th>Produk hasil</th><th>Unit</th><th>Qty hasil</th><th>Modal/unit hasil</th><th>Catatan</th><th></th></tr></thead><tbody>{rows.map((row) => { const suggestion = suggestedCost(row); return <tr key={row.key} className="border-t"><td className="p-2 min-w-[220px]"><SearchableSelect size="sm" value={row.sourceProductUnitId} onChange={(val) => patchRow(row.key, { sourceProductUnitId: val })} placeholder="Pilih bahan curah..." options={sources.map((item) => ({ value: item.productUnitId, label: item.productName, sublabel: `Tersedia: ${item.availableQty} ${item.unitName}` }))}/></td><td className="bg-slate-100 px-3 text-center font-bold text-slate-500">{productMap.get(row.sourceProductUnitId)?.unitName ?? '-'}</td><td><FormattedNumberInput allowDecimal value={row.sourceQuantity || 0} onChange={(val) => patchRow(row.key, { sourceQuantity: val })} className="h-9 w-24 rounded border text-center font-medium"/></td><td><ArrowRight className="mx-auto h-4 w-4 text-slate-400"/></td><td className="p-2 min-w-[220px]"><SearchableSelect size="sm" value={row.resultProductUnitId} onChange={(val) => patchRow(row.key, { resultProductUnitId: val })} placeholder="Pilih produk repack..." options={results.map((item) => ({ value: item.productUnitId, label: `${item.productName} · ${item.unitName}` }))}/></td><td className="bg-slate-100 px-3 text-center font-bold text-slate-500">{productMap.get(row.resultProductUnitId)?.unitName ?? '-'}</td><td><FormattedNumberInput allowDecimal value={row.resultQuantity || 0} onChange={(val) => patchRow(row.key, { resultQuantity: val, appliedUnitCost: undefined })} className="h-9 w-24 rounded border text-center font-medium"/></td><td className="p-2"><FormattedNumberInput min={0} value={row.appliedUnitCost ?? suggestion ?? ''} onChange={(val) => patchRow(row.key, { appliedUnitCost: val || undefined })} className="h-9 w-32 rounded border px-2 text-xs"/><p className="mt-1 text-[10px] text-slate-400">Saran {suggestion?.toLocaleString('id-ID') ?? '-'}</p></td><td><input value={row.note} onChange={(e) => patchRow(row.key, { note: e.target.value })} className="h-9 w-full rounded border px-2" placeholder="Opsional"/></td><td><button onClick={() => setRows((current) => current.filter((item) => item.key !== row.key))} className="p-2 text-rose-500"><Trash2 className="h-4 w-4"/></button></td></tr>; })}</tbody></table></div><div className="mt-3 flex flex-col gap-2 border-t pt-3 sm:flex-row sm:justify-between"><Button variant="outline" onClick={() => setRows((current) => [...current, emptyRow()])}><Plus className="mr-2 h-4 w-4"/>Tambah Baris</Button><div className="flex gap-2"><Button variant="outline" onClick={() => setFormOpen(false)}>Batal</Button><Button disabled={saving} onClick={() => void save()} className="bg-emerald-600 text-white">{saving && <RefreshCw className="mr-2 h-4 w-4 animate-spin"/>}Proses Sekarang</Button></div></div></DialogContent></Dialog>
