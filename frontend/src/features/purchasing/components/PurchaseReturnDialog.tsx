          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
            ...
            {view === 'detail' && selectedReturn && (
              <div className="space-y-4">
                ...
                {selectedReturn.status === 'READY' && selectedReturn.resolutionType === 'CASHBACK' && <div className="grid max-w-2xl gap-3 md:grid-cols-2"><div><Label>Metode Cashback</Label><Select value={cashbackMethod} onValueChange={(value) => { if (value === 'CASH' || value === 'TRANSFER') { setCashbackMethod(value); setFinancialAccountId(defaultFinancialAccount(accounts, value)); } }}><SelectTrigger className="w-full"><SelectValue>{cashbackMethod === 'CASH' ? 'Tunai' : 'Transfer'}</SelectValue></SelectTrigger><SelectContent className="bg-white"><SelectItem value="CASH">Tunai</SelectItem><SelectItem value="TRANSFER">Transfer</SelectItem></SelectContent></Select></div><div><Label>Akun Penerima Cashback</Label><SearchableSelect value={financialAccountId} onChange={setFinancialAccountId} placeholder="Pilih kas/bank..." options={accounts.map((account) => ({ value: account.financialAccountId, label: `${account.accountName} · ${account.accountType}` }))} /></div></div>}
                {selectedReturn.status === 'READY' && selectedReturn.resolutionType === 'NEXT_INVOICE_DEDUCTION' && <div className="max-w-md"><Label>Faktur Tempat Potongan Diterapkan</Label><SearchableSelect value={appliedInvoiceId} onChange={setAppliedInvoiceId} placeholder="Pilih PI berikutnya..." options={invoiceOptions.map((invoice) => ({ value: invoice.purchaseInvoiceId, label: `${invoice.purchaseInvoiceNumber} · ${invoice.invoiceDate.slice(0, 10)}` }))} /></div>}
