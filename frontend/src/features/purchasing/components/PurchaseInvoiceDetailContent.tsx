  ...
  return (
    <div className="flex-1 space-y-6 overflow-y-auto pr-1 text-xs custom-scrollbar">
      ...
      <div>
        ...
        {detail.payments.length === 0 ? <p className="rounded border bg-slate-50 p-3 text-center italic text-slate-400">Belum ada riwayat pembayaran yang dicatat.</p> : <div className="max-h-[220px] overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-inner custom-scrollbar"><table className="w-full border-collapse text-left"><thead className="sticky top-0 z-10 border-b border-emerald-200 bg-emerald-50 text-emerald-800"><tr><th className="p-3">Tanggal Bayar</th><th className="p-3">Akun Kas / Bank</th><th className="p-3">Metode</th><th className="p-3">No. Ref / Bukti</th><th className="p-3 text-right">Nominal (Rp)</th></tr></thead><tbody>{detail.payments.map((payment) => <tr key={payment.purchasePaymentId} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3">{payment.paymentDate.slice(0, 10)}</td><td className="p-3 font-bold">{payment.accountName}</td><td className="p-3 font-medium uppercase">{payment.paymentMethod}</td><td className="p-3 text-slate-500">{payment.referenceNumber || '-'}</td><td className="p-3 text-right font-extrabold text-emerald-600">Rp {payment.paymentAmount.toLocaleString('id-ID')}</td></tr>)}</tbody></table></div>}
      </div>

      <div className="flex justify-end rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm"><div className="w-80 space-y-2"><div className="flex justify-between text-slate-500"><span>Diskon Global:</span><span>Rp {detail.discountAmount.toLocaleString('id-ID')}</span></div><div className="flex justify-between border-t border-slate-200 pt-1.5 text-sm font-extrabold text-slate-800"><span>Grand Total:</span><span>Rp {detail.invoiceTotal.toLocaleString('id-ID')}</span></div><div className="flex justify-between font-bold text-emerald-700"><span>Total Dibayar:</span><span>Rp {detail.paidAmount.toLocaleString('id-ID')}</span></div><div className="flex justify-between border-t border-slate-200 pt-1.5 text-sm font-bold text-rose-600"><span>Sisa Hutang:</span><span>Rp {detail.outstandingAmount.toLocaleString('id-ID')}</span></div></div></div>
    </div>
  );
}
